...
const strategyByMode: Record<AccountVisualMode, VisualStrategy> = {
  ...
  museum: {
    ...
    avoid: ["不要伪造馆藏和展品来源", "不要违反馆方拍摄规则", "不要误导展期票务", "避免侵犯观众肖像权和隐私"]
  },
  product: {
...
const weddingVisualStrategy: VisualStrategy = {
  ...
  avoid: ["不要伪造真实婚礼案例", "避免侵犯新人/宾客肖像权和隐私", "不要夸大落地效果", "不要虚构价格档期和场地信息", "不要把 AI 图当真实案例"]
};
...
function modeCopy(accountType: string) {
  ...
  const copy: Record<AccountVisualMode, {
    ...
    heritage: {
      ...
      factRule: "禁止伪造传承人、仪式、民族/宗教元素、作品来源和活动现场，并保护人物隐私。",
    ...
    food: {
      sourceHint: "优先按文件名识别真实菜品图和环境图；图片文件名应尽量就是菜品名或环境名，例如 清蒸鲈鱼.jpg、竹林土鸡.jpg、包间.jpg、门头.jpg、停车场入口.jpg。指定真实图片默认只做照片级精修。",
      ...
      defaultStyleBrief: ["封面必须以真实主推菜图为锚点，需要时可直接排入准确短字。", "图集按“主推菜 / 菜品细节 / 套餐或第二道菜 / 内部环境 / 外部门头 / 交通指南漫画”组织。", "图片文件名应使用菜品名、环境名或交通节点名，方便龙虾自动识别。", "AI 图生图只做轻度优化和补图，不改变真实出品。"],
    ...
    service: {
      ...
      textRule: "流程图讲服务步骤；设备/资质图讲信任；信息卡讲价格、周期、预约、禁忌和隐私边界。",
      factRule: "禁止伪造顾客案例、效果对比、资质证书和服务结果，并保护顾客隐私。",
...
function modeCopyForAccount(account: Account) {
  ...
  return {
    ...
    factRule: "禁止伪造真实新人案例、宾客反馈、价格、档期、场地、套餐、花材成本和婚礼效果，并保护肖像权与隐私；未确认信息写待确认。",
    ...
    layoutRules: ["封面：婚礼细节占主体，短字写“细节名 + 风格关键词 + 备婚收藏理由”。", "细节拆解图：用 2-4 个短标签标出色系、材质、花材、层次、场景作用。", "关系图：展示该细节在全场婚礼中的位置，帮助新人理解落地效果。", "信息卡：列出适合婚礼风格、预算/档期/场地待确认项、适合人群和咨询问题。", "不要在图上暴露新人/宾客隐私、手机号、真实合同价格或其他敏感信息。"]
  };
}
...
${usesSelectedAssets ? "最后只列真实性和业务信息的人工核验项，不推荐其他素材或补拍内容。" : "最后列：必须补拍或人工确认的素材缺口。"}`;
}
