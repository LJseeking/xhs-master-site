...
export function buildBatchImagePostsPrompt(account: BatchPostAccount, options: BatchPostOptions) {
  ...
  const assetsDir = compact(options.openclawAssetsDir || account.assetsPath, "请填写龙虾可访问的图片文件夹");
  const specifiedImages = compact(options.openclawImagePaths, "未指定，默认扫描素材文件夹内全部图片。");
...
## 当前执行模式
只读研究 + 本地图片分析 + 生成批量帖子方案。不得发布、评论、点赞、收藏、关注或私信。

## 任务目标
客户会提供一批真实素材图片。请你先读取图片，识别其中最适合小红书表达的主体、细节、场景和可核验信息；再只读研究小红书全国范围内同类型热门内容；最后把“真实图片特点”和“全国同行爆款表达方式”合并，直接输出 ${weeks} 周小红书批量帖子方案（${targetCount}）。
...
export function buildBatchImagePostsCommands(account: BatchPostAccount, options: BatchPostOptions & { promptFile: string }) {
  ...
  const assetsDir = options.openclawAssetsDir || account.assetsPath || "填写龙虾可访问的图片文件夹";
  ...
  return [
    {
      ...
      description: `读取素材文件夹，先识别可写图片，再结合全国同类型爆款研究，输出 ${options.weeks === 2 ? "两周" : "一周"}批量帖子方案。`,
