...
function commandCopy(account: { accountType: string; name: string; personaBase: string; contentDirections: string; materialCondition: string; businessGoals: string; targetUsers: string }) {
  if (isWeddingAccount(account)) {
      return {
        source: "选择婚礼素材库图片",
        image2: "让龙虾直接读取婚礼素材库图片，先识别婚礼蛋糕、花艺、仪式区、迎宾区、桌花、席位卡、灯光布幔等细节，再根据逐张 Prompt 生成细节拆解图和信息卡。",
  ...
  const copies = {
    culture_tourism: {
      ...
      image2: "让龙虾直接读取文旅素材库图片，根据逐张 image2 Prompt 生成目的地图生图、动线图和服务信息卡底图。",
    ...
    heritage: {
      source: "选择民俗/非遗素材库图片",
      image2: "让龙虾直接读取民俗非遗素材库图片，根据逐张 image2 Prompt 生成工艺细节图生图、流程图和体验预约信息卡。",
      draftCategory: "调用已有民俗/非遗素材改图",
      draft: "读取素材库图片，优先识别真实工艺、作品、活动现场和授权人物图，再规划图生图、信息卡和图集顺序。",
      safety: "命令只作建议；确保肖像、作品和活动素材授权清楚，文化禁忌和表达边界需要人工核验。"
    },
    stay: {
      source: "选择房型/空间素材库图片",
      image2: "让龙虾直接读取住宿素材库图片，根据逐张 image2 Prompt 生成空间轻处理、设施说明和价格预订信息卡。",
    ...
    food: {
      source: "选择餐厅素材库图片",
      image2: "让龙虾直接读取餐厅素材库图片，优先按文件名识别菜品和环境，生成主推菜图生图、环境轻处理和交通指南漫画/信息卡。",
    ...
    outdoor: {
      source: "选择路线/现场素材库图片",
      image2: "让龙虾直接读取路线/现场素材库图片，根据逐张 image2 Prompt 生成现场图生图、路况轻处理和信息卡底图。",
    ...
    museum: {
      source: "选择展品/展厅素材库图片",
      image2: "让龙虾直接读取展馆研学素材库图片，根据逐张 image2 Prompt 生成展品图轻处理、观展动线和预约票务信息卡。",
      draftCategory: "调用已有展馆/研学素材改图",
      draft: "读取素材库图片，优先识别真实展品授权图、展厅图、导览图和票务截图，再规划图集顺序。",
      safety: "命令只作建议；确保展期、票务、拍摄规则和展品版权需要人工核验。"
    },
    product: {
      source: "选择产品/包装素材库图片",
      image2: "让龙虾直接读取产品素材库图片，根据逐张 image2 Prompt 生成产品图生图、工艺场景和规格价格信息卡。",
    ...
    service: {
      source: "选择服务/空间素材库图片",
      image2: "让龙虾直接读取本地服务素材库图片，根据逐张 image2 Prompt 生成服务流程图、设备资质图和价格预约信息卡。",
      draftCategory: "调用已有本地服务素材改图",
      draft: "读取素材库图片，优先识别真实空间、服务流程、设备资质和授权案例，再规划图集顺序。",
