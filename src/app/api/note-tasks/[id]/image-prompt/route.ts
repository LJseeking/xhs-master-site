...
完成后必须确认每张成品图都位于 \`$ACCOUNT_ASSETS_DIR\`，再按图文发布顺序，把这些成品图的本地绝对路径逐行写入 \`$TASK_DIR/image-paths.txt\`。清单中不得写入任务临时目录或其他账号目录中的图片。同时返回每张成品图的 \`local_path\`、图片序号、实际使用的 Prompt 和失败项。使用已有图片时还要返回对应原图。不要发布小红书内容。
